...
VSCode 支持用户自定义代码片段（参阅：[[Using VSCode snippets to set up code blocks]])，因此这一功能也可以在 Foam 中使用。在 Foam 笔记库根目录下 **.vscode** 路径创建 `foam-snippets.code-snippets` 文件以创建用户片段（该文件本质上是一个 json 文件）。
